...
# ─── AYARLAR ──────────────────────────────────────────────────────────────────
RUN_OPTUNA      = False  # Önceki run: lr=0.079, num_leaves=74, max_depth=11, min_child=25
OPTUNA_TRIALS   = 30
TOP_N_FEATURES  = 100
MISSING_THRESH  = 0.80
...
# ══════════════════════════════════════════════════════════════════════════════
print(SEP)
print('  AlgoMed V4 — CatBoost Stacking + EK Interactions + Missing Patterns')
print(SEP)
...
if RUN_OPTUNA:
    ...
    print(f'  En iyi Optuna MCC : {study.best_value:.4f}')
...
history = [
    ...
    ('V4', 'CatBoost Stacking + EK Interact + Patt.', best_f1, master_mcc, master_pr),
]
...
for ver, model, f1, mcc, pr in history:
    ...
    marker = ' <-- V4' if ver == 'V4' else ''
    print(f'  {ver:<4} {model:<42} {f1:>8.4f} {mcc:>8.4f} {pr_str:>8}{marker}')

v3_f1, v3_mcc = 0.8929, 0.5401
delta_f1  = best_f1   - v3_f1
delta_mcc = master_mcc - v3_mcc
print(f'\n  V4 - V3 Farkı: ΔF1={delta_f1:+.4f}  ΔMCC={delta_mcc:+.4f}')
if delta_f1 > 0:
    print('  ✅ V4, V3\'ü geçti — GitHub\'a push edilebilir!')
else:
    print('  ⚠️  V4 henüz V3\'ü geçemedi — parametreleri optimize et.')

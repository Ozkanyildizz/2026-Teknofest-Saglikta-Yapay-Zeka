...
print('  AlgoMed V6 — GPU Hızlandırılmış Stacking Ensemble (LGBM+XGB+CatBoost)')
...
# FN Analizinden gelen yeni risk özellikleri
...
print(">>> cross_val_predict başlıyor")
...
print(f"\nToplam False Negative: {len(fn_df)}")



fn_df.to_csv("false_negatives_v5.csv", index=False)



print("false_negatives_v5.csv kaydedildi")
...
print(fn_df["EK_7"].describe())



print("\nTüm Veri EK_7 Özeti")

print(df["EK_7"].describe())



print("\nFN missing_ratio_AL Özeti")

print(fn_df["missing_ratio_AL"].describe())



print("\nTüm Veri missing_ratio_AL Özeti")

print(df["missing_ratio_AL"].describe())



print("\nEn Sık AA Dönüşümleri (FN)")



print(

    fn_df.groupby(["AA_1","AA_2"])
...
fn_ek7_low = fn_df["EK_7"] < 3



print("AA eksik FN sayısı:", fn_aa_missing.sum())



print("EK_7 düşük FN sayısı:", fn_ek7_low.sum())



print(

    "Hem AA eksik hem EK_7 düşük:",

    (fn_aa_missing & fn_ek7_low).sum()
...
print("\nFN Alt Grup Dağılımı")



fn_ids = set(fn_df["Variant_ID"])
...
for name, path in {
    ...
    common = temp["Variant_ID"].isin(fn_ids).sum()
...
print("\nPAH False Negativeler")



pah_ids = set(

    pd.read_csv(PATHS["PAH"])["Variant_ID"]
